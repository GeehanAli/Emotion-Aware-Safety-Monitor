...
class SafetyMonitor:
    ...
    def train(self):
        ...
        print("🔄 Training Enhanced Safety Monitor...")
        print("📚 Including research-proven baselines from literature")
        
        X_train_vec, X_test_vec, X_train, X_test, y_train, y_test = self.prepare_data()
        
        # Train our main model
        print("🤖 Training our logistic regression model...")
    ...
    def compare_with_proven_baselines(self, X_train_vec, X_test_vec, y_train, y_test):
        ...
        print("🔬 Training SVM baseline (literature's best F1-score)...")
        ...
        print("🔬 Training MLP baseline (literature's best accuracy)...")
...
def main():
    """Main function to run the ML model without baseline import"""
    print("🎮 Emotion-Aware Safety Monitor - Research Edition")
    print("Building on established grooming detection literature...")
    print("=" * 60)
    
    print("Using previously measured baseline: Accuracy: 0.76, F1: 0.74")
    keyword_accuracy, keyword_f1 = 0.76, 0.74
    
    print("\nTraining our enhanced ML approach with research baselines...")
    ...
    print("🎯 FINAL SUMMARY")
    print("="*60)
    print(f"🔍 Keyword Filter:    Accuracy: {keyword_accuracy:.4f}, F1: {keyword_f1:.4f}")
    print(f"🤖 Our ML Model:      Accuracy: {our_accuracy:.4f}, F1: {our_f1:.4f}")
    print(f"📈 Improvement:       F1-Score: +{(our_f1 - keyword_f1):.4f}")
    ...
    for msg in test_messages:
        ...
        print(f"{confidence_color} '{msg}' → {result['prediction']} (conf: {result['confidence']:.2f})")
